 ...
 * </div>
 */
function insertNewPost(description, photoURL, price, city, condition, link, tooltipText) {
  ...
  var html = `<div class="post" data-price="${price}" data-city="${city}" data-condition="${condition}" data-tooltipText="${tooltipText}" data-link="${link}">
  <a href=${link} target="_blank">
      <div class="post-contents">
          <div class="post-image-container">
              <img src="${photoURL}" alt="${description}">
          </div>
          <div class="post-info-container" style="margin-left: 50%;">
...
function doFilterUpdate() {
  ...
  allPosts.forEach(function (post) {
    if (postPassesFilters(post, filters)) {
      insertNewPost(post.description, post.photoURL, post.price, post.city, post.condition, post.link, post.tooltipText);
    }
  });
...
function parsePostElem(postElem) {
  ...
  var post = {
    ...
    tooltipText: postElem.getAttribute('data-tooltipText')
  };
